declare const enum ScrollConsts {
  ...
  maxCalibration = 1.6,
  minCalibration = 0.5,
  SpeedChangeInterval = 47,
  minDuration = 120,
  durationScaleForAmount = 20, // 120 ~= 20 * ln(400)
  maxS = 1.05,
  minS = 0.95,
  delayToChangeSpeed = 75,
  tickForUnexpectedTime = 17,
  firstTick = 17,
...
import {
  ...
  getVisibleBoundingRect_,
  getVisibleClientRect_,
  dimSize_,
  scrollingTop,
  set_scrollingTop,
  isSelARange,
...
export const setNewScrolling = (el: Element | null): void => {
  currentScrolling = OnFirefox
    ? weakRef_ff(el as SafeElement | null, kElRef.currentScrolling)
    : weakRef_not_ff!(SafeEl_not_ff_!(el));
  cachedScrollable = null;
};
export function set_cachedScrollable(_newCurSc: typeof currentScrolling): void {
  cachedScrollable = _newCurSc;
}
...
let performAnimate = (
  newEl: SafeElement | null,
  newDi: ScrollByY,
  newAmount: number,
  newOpts?: CmdOptions[kFgCmd.scroll],
): ReturnType<VApiTy["$"]> => {
  const hasNewScrollEnd =
    (OnChrome &&
      (Build.MinCVer >= BrowserVer.MinEnsuredScrollend ||
        chromeVer_ > BrowserVer.MinEnsuredScrollend - 1)) ||
    ((OnFirefox ||
      (OnChrome &&
        (Build.MinCVer >= BrowserVer.MinScrollEndForInstantScrolling ||
          chromeVer_ > BrowserVer.MinScrollEndForInstantScrolling - 1))) &&
      "on" + kSE in Image.prototype);
  let amount: number,
    sign: number,
    calibration: number,
    di: ScrollByY,
    duration: number,
    element: SafeElement | null,
    ...
    min_delta = 0,
    running = 0,
    flags: kScFlag & number,
    calibTime: number,
    lostFrames: number,
    totalTick: number,
    styleTop: SafeElement | HTMLElement | null | undefined,
    onFinish: ((succeed: number) => void) | 0 | undefined,
    wait2: number | boolean | null | undefined,
    timer: ValidTimeoutID = TimerID.None,
    padding: number,
    animate = (newRawTimestamp: number): void => {
      const continuous = keyIsDown > 0;
      let rawElapsed = newRawTimestamp - rawTimestamp;
      let newTimestamp = newRawTimestamp,
        elapsed: number,
        delay2: number;
      ...
      if (!timestamp) {
        ...
        if (
          OnFirefox &&
          rawElapsed > ScrollConsts.FirefoxMinFakeInterval - 1 &&
          ...
          if (
            preventPointEvents > ScrollConsts.MinLatencyToAutoPreventHover - 1 &&
            rawElapsed > preventPointEvents &&
            min_delta > 4 &&
            rawElapsed > min_delta * 1.8 &&
            ++lostFrames > 2
          ) {
            preventPointEvents = 2;
            toggleAnimation!(1);
          }
          min_delta = !rawTimestamp
            ? min_delta
            : !min_delta
              ? rawElapsed < 3 && rawTimestamp !== timestamp
                ? 0
                : min_(
                    max_(rawTimestamp !== timestamp ? 11 : 6, rawElapsed + 0.1),
                    ScrollConsts.firstTick,
                  )
              : rawElapsed < min_delta * 0.7
                ? 0.85 * min_delta
                : rawElapsed < min_delta * 1.3
                  ? (min_delta + rawElapsed) / 2
                  : (min_delta * 7 + rawElapsed) / 8;
        }
      }
      ...
      if (ScrollConsts.DEBUG & 1) {
        console.log(
          "rawOld>rawNew: +%o = %o ; old>new: +%o = %o ; elapsed: +%o = %o; min_delta = %o (%o fps)",
      ...
      if (continuous) {
        if (totalElapsed >= ScrollConsts.delayToChangeSpeed) {
          if (totalElapsed > minDelay) {
            keyIsDown = keyIsDown > elapsed ? keyIsDown - elapsed : 0;
          }
          calibTime += elapsed;
          if (
            ScrollConsts.minCalibration <= calibration &&
            calibration <= ScrollConsts.maxCalibration &&
            calibTime > ScrollConsts.SpeedChangeInterval
          ) {
            const calibrationScale = ScrollConsts.calibrationBoundary / amount / calibration;
            calibration *=
              calibrationScale > ScrollConsts.maxS
                ? ScrollConsts.maxS
                : calibrationScale < ScrollConsts.minS
                  ? ScrollConsts.minS
                  : 1;
            calibTime = 0;
          }
      ...
      if (min_delta && elapsed < 1.2 * min_delta && elapsed > 0.9 * min_delta) {
        ...
        const step =
          fps_test < 95 ? (fps_test < 55 ? 1 : 15) : fps_test > 149 ? (fps_test < 195 ? 5 : 10) : 0;
        near_elapsed =
          1e3 /
          (step
            ? (math.round(fps_test / step) || 1) * step
            : fps_test < 110
              ? 100
              : fps_test < 132
                ? 120
      ...
      if (!continuous || ((totalDelta < amount || flags & kScFlag.TO) && totalElapsed < minDelay)) {
        delta = max_(
          0,
          min_(delta + 2 > amount - totalDelta && delta > 4 ? amount : delta, amount - totalDelta),
        );
      }
      if (delta > 0) {
        const wanted = delta,
          beforeScPos = ScrollConsts.DEBUG & 2 ? dimSize_(element, kDim.scPosX + di) : -1,
          visual = !(ScrollConsts.DEBUG & 2 && element === scrollingTop)
            ? null
            : !OnFirefox || Build.MinFFVer >= FirefoxBrowserVer.MinEnsured$visualViewport$
              ? visualViewport
              : (window as {} as typeof globalThis).visualViewport;
        const beforeViewOffset =
          (ScrollConsts.DEBUG & 2 && visual?.[di ? "offsetTop" : "offsetLeft"]) || 0;
        const beforeViewPos =
          (ScrollConsts.DEBUG & 2 && visual?.[di ? "pageTop" : "pageLeft"]) || 0;
        // here should keep safe even if there're bounce effects
        delta = performScroll(
          element,
          di,
          sign * max_(scale, (delta > 4 ? math.round : math.ceil)(delta)),
          beforePos,
        );
        if (ScrollConsts.DEBUG & 2) {
          const near_fps = 1e3 / near_elapsed,
            resScPos = dimSize_(element, kDim.scPosX + di);
          const resViewOffset =
            (ScrollConsts.DEBUG & 2 && visual?.[di ? "offsetTop" : "offsetLeft"]) || 0;
          const resViewPos = (ScrollConsts.DEBUG & 2 && visual?.[di ? "pageTop" : "pageLeft"]) || 0;
          console.log(
            "do scroll: e := %o + ~(%o fps %s %o px = %o) => %o ; " +
              "r := mem %o / sc %o / view %o/%o + ~ %s%o / d %o => %o / %o / %o/%o; %o, %o",
            /* e := */ norm(totalDelta),
            /* + ~( */ near_fps > 300 ? -1 : norm(near_fps) /* fps */,
            padding > 0 ? "-" : "+",
            norm(abs_(padding)),
            /* px = */ norm(wanted) /* ) */,
            /* => */ norm(totalDelta + sign * (wanted > 4 ? math.round : math.ceil)(wanted)),
            /* r := mem */ norm(beforePos),
            /* / sc */ norm(beforeScPos),
            /* / view */ norm(beforeViewOffset),
            /* / */ norm(beforeViewPos),
            /* + ~ */ sign > 0 ? "" : "-",
            norm(wanted),
            /* / d */ norm(delta),
            /* => */ norm(beforePos + delta),
            /* / */ norm(resScPos),
            /* / */ norm(resViewOffset),
            /* / */ norm(resViewPos),
            /* am= */ norm(amount),
      ...
      if (delta && (!onFinish || totalDelta < amount)) {
        ...
        if (!OnFirefox) {
          ...
          checkCurrent(el2);
        } else if (hasNewScrollEnd) {
          // avoiding async stack tracing on Firefox 114
          const notEl: boolean = !el2 || el2 === scrollingEl_();
          void dispatchAsync_(notEl ? doc : el2!, newEvent_(kSE, 1, 1, !notEl)).then(
            checkCurrent.bind(0, el2),
          );
        } else {
          checkCurrent(el2);
        }
    ...
    resumeAnimation = (): void => {
      ...
      flags & kScFlag.TO &&
        amount > fgCache.t &&
        (amount = min_(amount, dimSize_(element, di + kDim.viewW) / 2) | 0);
      running = running || rAF_(animate);
    };
  toggleAnimation = (scrolling?: BOOL | 4): void => {
    if (scrolling === 4) {
      wait2 || (timer && (clearTimeout_(timer), resumeAnimation()));
      return;
    }
    if (!scrolling) {
      if (ScrollConsts.DEBUG) {
        console.log(
          ">>> [animation] end after %o ms / %o px / %o ticks",
          norm(totalElapsed),
          norm(totalDelta),
          totalTick,
        );
    ...
    if (!isHTML_()) {
      ...
      scrolling
        ? ourDialogEl_ || addElementList([], [0, 0], 4)
        : ourDialogEl_ !== hint_box && removeDialog_();
    } else {
      el = (
        scrolling ? (OnFirefox ? docEl_unsafe_() : SafeEl_not_ff_!(docEl_unsafe_())) : styleTop
      ) as (SafeElement & ElementToHTMLOrForeign) | null;
      style = el && el.style;
  ...
  performAnimate = (newEl1, newDi1, newAmount1, options): ReturnType<VApiTy["$"]> => {
    ((amount = max_(1, newAmount1 > 0 ? newAmount1 : -newAmount1)),
      (calibration = 1.0),
      (di = newDi1));
    flags = options ? options.f! | 0 : 0;
    wait2 = options && options.wait;
    duration = math.round(
      max_(1, (ScrollConsts.durationScaleForAmount * math.log(amount)) / ScrollConsts.minDuration) *
        fgCache.u,
    );
    element = newEl1;
    ...
    minDelay =
      keyboard[0] + max_(keyboard[1], ScrollConsts.DelayMinDelta) + ScrollConsts.DelayTolerance;
    (preventPointEvents === 2 || (preventPointEvents === 1 && !isSelARange(getSelection_()))) &&
      toggleAnimation!(1);
    if ((OnChrome || OnFirefox) && hasNewScrollEnd) {
      elementRoot = element
        ? getRootNode_mounted(element as EnsuredMountedElement & typeof element)
        : doc;
    ...
    if (ScrollConsts.DEBUG) {
      console.log(
        "%c[animation]%c start with axis = %o, amount = %o, dir = %o, duration = %o, min_delta = %o%s %s",
        "color: #1155cc",
        "color: auto",
        di ? "y" : "x",
        norm(amount),
        ...
        `flags = ${flags}, wait2 = ${wait2 ?? -1}`,
      ); // lgtm [js/useless-expression]
    }
    if (OnFirefox) {
      const visual_ff =
        element !== scrollingTop
          ? null
          : Build.MinFFVer >= FirefoxBrowserVer.MinEnsured$visualViewport$
            ? visualViewport
            : (window as {} as typeof globalThis).visualViewport;
      if (visual_ff && visual_ff.scale > 1) {
        ...
        const lacked =
          max_(2, min_(amount, 30)) -
          (sign < 0 ? scPos : dimSize_(element, kDim.scrollW + di) - oriSize - scPos);
        if (lacked > 0) {
          const offset = di ? visual_ff!.offsetTop : visual_ff!.offsetLeft;
          const fromScaledViewEnd =
            sign < 0 ? oriSize - visual_ff!.height - offset + lacked : offset + lacked;
          if (fromScaledViewEnd > 0) {
            const stat_ = (): [scrollLeftTop: number, visualOffsetLeftTop: number] | null =>
              ScrollConsts.DEBUG & 2
                ? [dimSize_(element, kDim.scPosX + di), visual_ff![di ? "offsetTop" : "offsetLeft"]]
                : null;
            const st1 = stat_();
    ...
    return (
      (options &&
        (options.$then || options.$else) &&
        ((defer = promiseDefer_()), (onFinish = defer.r), defer.p)) ||
      0
    );
  };
...
export const $sc: VApiTy["$"] = (element, di, amount, options): ReturnType<VApiTy["$"]> => {
  ...
  if (hasSpecialScrollSnap(element)) {
    ...
    (options && options.smooth != null ? options.smooth : fgCache.s) &&
...
export const activate = (options: CmdOptions[kFgCmd.scroll] & SafeObject, count: number): void => {
  ...
  if (checkHidden(kFgCmd.scroll, options, count)) {
    return;
  }
  if (tryNestedFrame(kFgCmd.scroll, options, count)) {
    return;
  }
  ...
  executeScroll(
    di,
    count,
    dest ? (fromMax ? kScFlag.toMax : kScFlag.toMin) : (kScFlag.scBy as never),
    options.view as undefined,
    options,
    oriCount,
  );
 ...
 */
export const executeScroll: VApiTy["c"] = function (
  di: ScrollByY,
  amount0: number,
  flags: kScFlag & number,
  factor?: NonNullable<CmdOptions[kFgCmd.scroll]["view"]> | undefined,
  options?: CmdOptions[kFgCmd.scroll],
  oriCount?: number,
  ...
  const callOtherFrame = (dir: 0 | 2): void => {
    visitor!.n += 1 - dir;
    (core as IfTrueOr<typeof core, null>)!.c(
      di,
      amount0,
      flags as 0,
      factor,
  ...
  const toFlags = flags & (kScFlag.TO | kScFlag.INC),
    toMax = (toFlags - kScFlag.TO) as BOOL;
  let core: ReturnType<typeof getParentVApi> | false;
  {
    const childFrame =
      ((visitor ||= { v: joined, n: 0 }), !visitorDi) && derefInDoc_(currentScrolling);
  ...
  const element = findScrollable(
    di,
    toFlags ? toMax || -1 : amount0,
    outer > 0 ? (oriCount! <= outer ? abs_(oriCount!) : 1) : -outer | 0,
    options &&
      (options.scroll
        ? options.scroll === "force"
        : options.evenIf != null
          ? ((options.evenIf & kHidden.OverflowHidden) as 0 | 2)
          : null),
  ...
  let viewSize: number | undefined,
    toDoInSelf: BOOL = 1,
    amount =
      elementIsTop && isTopScrollable < 1
        ? 0
        : !factor
          ? (!di &&
            amount0 &&
            element &&
            dimSize_(element, kDim.scrollW) <=
  ...
  if (toFlags) {
    ...
    if (ScrollConsts.DEBUG & 8) {
      console.log(
        "[scrollTo] cur=%o view=%o max=%o amount=%o, so final amount=%o",
        norm(curPos),
        norm(viewSize),
        norm(max),
        norm(oldAmount),
  ...
  if (
    ...
    options &&
    !injector &&
    !(options as OptionsWithForce).$forced &&
  ...
  if (toFlags && elementIsTop && amount) {
    ...
    if (
      !joined &&
      options &&
      (options as Extract<typeof options, { dest: string }>).sel === "clear"
    ) {
      resetSelectionToDocStart();
    }
  }
  ...
  preventPointEvents =
    keepHover === !1
      ...
      : keepHover === "never"
        ? 2
        : keepHover === "auto"
          ? ScrollConsts.MinLatencyToAutoPreventHover
          : keepHover! > ScrollConsts.MinLatencyToAutoPreventHover - 1
            ? (keepHover as ScrollConsts.MinLatencyToAutoPreventHover)
            : 0;
  ...
  if (ret && isTY(ret, kTY.obj)) {
    void ret.then((succeed): void => {
      runFallbackKey(options!, succeed ? 0 : 2);
    });
  } else if (toDoInSelf) {
...
export const scrollTick: VApiTy["k"] = (willContinue: 0 | 1 | 2 | 5): void => {
  if (ScrollConsts.DEBUG & 4 && (keyIsDown || willContinue === 1)) {
    console.log(
      "update keyIsDown from",
      norm(keyIsDown),
      "to",
      willContinue & 1 ? maxKeyInterval : 0,
      "@",
...
export const beginScroll = (
  eventWrapper: 0 | Pick<HandlerNS.Event, "e">,
  key: string,
  keybody: kChar,
): void => {
...
const findScrollable = (
  ...
  amount: number,
  outer: number,
  evenOverflowHidden?: boolean | 0 | 2 | null | undefined,
  scrollable?: string,
): SafeElement | null => {
  const selectFirst = (info: ElementScrollInfo): ElementScrollInfo | null | undefined => {
    let cur_el: SafeElement | HTMLFormElement = info.e,
      type: 0 | 1 | -1;
    if (
      dimSize_(cur_el, kDim.elClientH) + 3 < dimSize_(cur_el, kDim.scrollH) &&
      ((type = shouldScroll_s(cur_el, cur_el !== top && cur_el !== body ? selectFirstType : di, 1)),
      type > 0 ||
        (!type && dimSize_(cur_el, kDim.positionY) > 0 && doesScroll(cur_el, kDim.byY, 0)))
    ) {
      return info;
    }
    ...
    for (let _ref = cur_el.children, _len = _ref.length > 50 ? 0 : _ref.length; 0 < _len--; ) {
      ...
      if (visible) {
        const height = visible.b - visible.t,
    ...
    return children.reduce(
      (cur, info1) => cur || selectFirst(info1),
      null as ElementScrollInfo | null | undefined,
    );
  };
  ...
  const fullscreen = fullscreenEl_unsafe_(),
    top: Element | null = fullscreen || scrollingTop,
    body = doc.body;
  const selectAncestor = (): void => {
    while (
      element !== top && (!fullscreen || IsAInB_(element as SafeElement, fullscreen))
        ? shouldScroll_s(
            <SafeElement>element,
            element === lastCachedScrolled ? ((di + 2) as 2 | 3) : di,
            amount,
          ) < 1 || --outer > 0
        : ((element = top), 0)
    ) {
  ...
  if (!element) {
    ...
    if (selector) {
      element = OnFirefox
        ? (querySelector_unsafe_(selector) as SafeElement | null)
        : SafeEl_not_ff_!(querySelector_unsafe_(selector));
      element =
        element && (!fullscreen || IsAInB_(element as SafeElement, fullscreen)) ? element : null;
    }
  }
  if (!element && top && isSafeEl_(top)) {
    ...
    if (isTopScrollable < 1) {
      ...
      if (
        element &&
        getEditableType_(element) &&
        dimSize_(element, kDim.elClientH) < wndSize_() / 2
      ) {
    ...
    element =
      candidate && candidate.e !== top && (!activeEl || candidate.h > wndSize_() / 2)
        ? candidate.e
        : element || top;
...
const checkCurrent = (el: SafeElement | null): void => {
  ...
  if (el && (!cur || (cur !== el && !(IsAInB_(cur, el) && !isNotInViewport(cur))))) {
    ...
    const par: SafeElement =
      last && el !== last && last !== cur && IsAInB_(last, el) && !isNotInViewport(last)
        ? last
        : el;
    setNewScrolling(par);
...
export const scrollIntoView_s = (el: SafeElement | null, r2: Rect | null, dir: 0 | 1 | 2): void => {
  ...
  let { l, t, r, b } = rect1;
  if (r2) {
    ((l = min_(max_(l, r2.l), r)),
      (t = min_(max_(t, r2.t), b)),
      (r = max_(l, min_(r2.r, r))),
      (b = max_(t, min_(r2.b, b))));
  }
  ...
  for (
    ;
    el && (hasX || hasY);
    el = GetParent_unsafe_(el, PNType.RevealSlotAndGotoParent) as SafeElement | null
  ) {
...
export const makeElementScrollBy_ = (
  el: SafeElement | null | 0,
  hasX: number,
  hasY: number,
): void => {
  set_cropNotReady_(2);
  void (
    hasX &&
    (hasY ? performScroll : vApi.$)(
      el !== 0 ? el : findScrollable(kDim.byX, hasX, 0),
      kDim.byX,
      hasX,
    )
  );
...
export const shouldScroll_s = (
  element: SafeElement,
  di: BOOL | 2 | 3,
  amount: number,
): -1 | 0 | 1 => {
  const st = getComputedStyle_(element),
    overflow = di ? st.overflowY : st.overflowX;
  return (overflow === HDN && di < 2) ||
    overflow === "clip" ||
    st.display === NONE ||
    !isRawStyleVisible(st)
    ? -1
    : <BOOL>(
        +doesScroll(
          element,
          (di & 1) as BOOL,
          amount || +!dimSize_(element, kDim.scPosX + (di & 1)),
        )
      );
};
...
export const suppressScroll = (timedOut?: number): void => {
  timedOut =
    timedOut || (OnChrome && Build.MinCVer <= BrowserVer.NoRAFOrRICOnSandboxedPage && noRAF_old_cr_)
      ? 1
      : 0;
  ...
  if (!timedOut) {
    ...
    Build.BTypes & BrowserType.Edge
      ? Promise.resolve(suppressScroll).then(rAF_)
    ...
    const path = getEventPath(event),
      el =
        (!OnEdge &&
          (!OnChrome ||
            Build.MinCVer >=
              BrowserVer.MinOnFocus$Event$$Path$IncludeOuterElementsIfTargetInClosedShadowDOM ||
            Build.MinCVer >=
              BrowserVer.Min$Event$$Path$IncludeWindowAndElementsIfListenedOnWindow)) ||
